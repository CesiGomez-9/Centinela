...
<head>
    <meta charset="UTF-8">
    <title>Factura de Compra {{ $factura->numero_factura }}</title>
    <style>
...
<div class="card">
    ...
    <div class="card-header">
        ...
        <div class="fecha-creacion">Creado {{ $factura->created_at->diffForHumans() }}</div>
    </div>
    ...
    <div class="card-body">
        ...
        <table class="info-table">
            <tr>
                <td>
                    <div class="info-box">
                        <p><strong>Proveedor:</strong> {{ $factura->proveedor->nombreEmpresa ?? 'N/A' }}</p>
                ...
                <td>
                    <div class="info-box">
                        <p><strong>Factura de compra N°:</strong> {{ $factura->numero_factura ?? 'N/A' }}.</p>
                        <p><strong>Fecha de la factura:</strong> {{ \Carbon\Carbon::parse($factura->fecha ?? now())->format('d/m/Y') }}.</p>
                        <p><strong>Proveedor:</strong> {{ $factura->proveedor->nombreEmpresa ?? 'N/A' }}</p>
                        <p><strong>Forma de pago:</strong> {{ $factura->forma_pago ?? 'N/A' }}.</p>
                        <p><strong>Responsable:</strong> {{ $factura->empleado->nombre ?? 'N/A' }} {{ $factura->empleado->apellido ?? 'N/A' }}.</p>
                    </div>
                </td>
            </tr>
        </table>
            ...
            <div class="totals-box">
                <table class="totals-row">
                    <tr>
                        ...
                        <td class="value">{{ number_format($factura->importe_gravado ?? 0, 2) }}</td>
                    </tr>
                    <tr>
                        <td class="label">Importe Exento (Lps):</td>
                        <td class="value">{{ number_format($factura->importe_exento ?? 0, 2) }}</td>
                    </tr>
                    <tr>
                        <td class="label">Importe Exonerado (Lps):</td>
                        <td class="value">{{ number_format($factura->importe_exonerado ?? 0, 2) }}</td>
                    </tr>
                    <tr>
                        <td class="label">Subtotal (Lps):</td>
                        <td class="value">{{ number_format($factura->subtotal ?? 0, 2) }}</td>
                    </tr>
                    <tr>
                        <td class="label">ISV 15% (Lps):</td>
                        <td class="value">{{ number_format($factura->isv_15 ?? 0, 2) }}</td>
                    </tr>
                    <tr>
                        <td class="label">ISV 18% (Lps):</td>
                        <td class="value">{{ number_format($factura->isv_18 ?? 0, 2) }}</td>
                    </tr>
                    <tr class="total-final">
                        <td class="label">Total Final (Lps):</td>
                        <td class="value">{{ number_format($factura->totalF ?? 0, 2) }}</td>
                    </tr>
                </table>
            </div>
        </div>
