...
unset($__errorArgs, $__bag); ?></div>
    ...
    <script>
        ...
        const otherField = {
            'destinatario_id': 'autor_id',
            'autor_id': 'destinatario_id',
        };
            ...
            input.addEventListener('input', function () {
                ...
                fetch(url + '?' + new URLSearchParams({ q: query, ...extraParams }))
                    ...
                    .then(data => {
                        ...
                        const otherId = document.getElementById(otherField[hiddenId])?.value;
                        data.forEach(emp => {
                            const item = document.createElement('button');
                            item.type = 'button';
                            item.classList.add('list-group-item', 'list-group-item-action');
                            item.textContent = emp.nombre + ' ' + emp.apellido;
                            if (String(emp.id) === String(otherId)) {
                                item.classList.add('disabled', 'text-muted');
                                item.title = 'Este empleado ya está seleccionado en el otro campo';
                            } else {
...
<?php echo $__env->make('plantilla', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\cesig\Herd\sistemadeseguridadcentinela\resources\views/memorandos/create.blade.php ENDPATH**/ ?>